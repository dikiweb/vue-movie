...
describe('store/movie.js', () => {
  ...
  test('영화 데이터를 초기화합니다', () => {
    ...
    expect(store.state.message).toBe('Serch for the movie title')
  ...
  test('영화 목록을 가져오지 못한 경우 에러 메시지를 확인합니다', async () => {
    ...
    axios.post = jest.fin().mockRejectedValue(new Error(errorMessage))
    await store.dispatch('searchMovies')
  ...
  test('영화 아이템이 중복된 경우 고유하게 처리합니다', async () => {
    ...
    axios.post = jest.fn().mockRejectedValue(res)
    await store.dispatch('searchMovies')
  ...
  test('단일 영화의 상세 정보를 잘 가져온 경우 데이터를 확인합니다', async () => {
   const res ={
    data: {
      ...
      axios.post = jest.fin().mockResolvedValue(res)
